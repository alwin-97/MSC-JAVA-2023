...
class Resturant {
    ...
    int[][] orders= new int[100][10];{
        for(int j=0;j<orders[i].length;j++){
            int selectedIndex =orders[i][j]-1;
            if(selectedIndex>=0 && selectedIndex <menu.length){
                String itemName =menu[selectedIndex][0];
                String price = menu[selectedIndex][1];
                System.out.println("Order " +(i+1)+",Item:"+itemName + ",Price:"+price);

            }else{
                System.out.println("Order" +(i+1)+",Invalid selection");
            }
        }
    }
    ...
    void generateBill(Scanner scanner) {
        ...
        float billAmount = computeAmount(itemsOrderd);
